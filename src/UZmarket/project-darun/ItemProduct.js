...
export const ItemProduct = ({ productItems }) => {
  let { productItemsId } = useParams();

  let [counter, setCounter] = useState(1);
  let [on, setOne] = useState(true);
  let [images, setImages] = useState(productItems[productItemsId - 1].Img);
  let [search, setSearch] = useState(productItems[productItemsId - 1].Name);

  let item = productItems.find(i => i.id === productItemsId);
  ...
  const itemLocal = () => {
    if (localStorage.getItem("karzinaProduct")) {
      ...
      a.push(productItems[productItemsId - 1]);
      ...
      b.push(productItems[productItemsId - 1]);
      localStorage.setItem("karzinaProduct", JSON.stringify(b));
    }
  };

  const ImgList = (productItems) => {
    images = productItems[productItemsId - 1].Img;
    setImages(images);
  };

  const ImgList2 = (productItems) => {
    images = productItems[productItemsId - 1].Img2;
    setImages(images);
  };
  const ImgList3 = (productItems) => {
    images = productItems[productItemsId - 1].Img3;
    setImages(images);
  };
  const ImgList4 = (productItems) => {
    images = productItems[productItemsId - 1].Img4;
    setImages(images);
  };
          ...
          <Box display={"flex"}>
            <Box display={"flex"} justifyContent={"flex-start"} gap={"10px"}>
              <Box display={"flex"} flexDirection={"column"} gap={"10px"}>
                <Button
                  onClick={() => ImgList(productItems)}
                  sx={{ padding: "0px 0px" }}
                >
                  <img
                    src={productItems[productItemsId - 1].Img}
                    alt=""
                    width={"90px"}
                  />
                </Button>
                <Button
                  onClick={() => ImgList2(productItems)}
                  sx={{ padding: "0px 0px" }}
                >
                  <img
                    src={productItems[productItemsId - 1].Img2}
                    alt=""
                    width={"90px"}
                  />
                </Button>
                <Button
                  onClick={() => ImgList3(productItems)}
                  sx={{ padding: "0px 0px" }}
                >
                  <img
                    src={productItems[productItemsId - 1].Img3}
                    alt=""
                    width={"90px"}
                  />
                </Button>
                <Button
                  onClick={() => ImgList4(productItems)}
                  sx={{ padding: "0px 0px" }}
                >
                  <img
                    src={productItems[productItemsId - 1].Img4}
                    alt=""
                    width={"90px"}
                  />
                </Button>
            ...
            <Box flexDirection={"column"} ml={"100px"}>
              <Box display={"flex"} sx={{ opacity: "0.80" }}>
                <Box>
                  <Typography fontSize={"14px"} fontWeight={"500"} variant="p">
                    ...
                    {productItems[productItemsId - 1].StarT}
                  </Typography>
                </Box>
                <Box ml={"20px"}>
                ...
                <Box display={"flex"}>
                  ...
                  <Box mt={"22px"}>
                    <Typography variant="p" fontSize={"20px"} color={"#7000ff"}>
                      {productItems[productItemsId - 1].Qolgani}
                    </Typography>
                  </Box>
                </Box>
              </Box>
              ...
              <Box display={"flex"}>
                <Typography variant="p" fontSize={"23px"} fontWeight={"600"}>
                  {productItems[productItemsId - 1].NewPrise} so'm
                </Typography>
                <Box
                ...
                >
                  <Typography variant="p" fontSize={"15px"} fontWeight={"400"}>
                    {productItems[productItemsId - 1].OldPrise} so'm
                  </Typography>
                </Box>
                <Button
                  variant="contained"
              ...
              <Box mt={"10px"}>
                <Button
                  onClick={() => itemLocal(productItemsId.length)}
                  variant="contained"
                  sx={{
                    padding: "20px 60px",
                    bgcolor: "#7000ff",
              ...
              >
                ...
                <Typography mt="3px" ml="5px">
                  Bu haftada 6103 kishi sotib oldi
                </Typography>
              </Box>
            </Box>
          </Box>
